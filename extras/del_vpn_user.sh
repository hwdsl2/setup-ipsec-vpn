...
record_exists() {
  ...
  local file=$2
  VPN_RECORD_PREFIX="$record_prefix" awk '
    index($0, ENVIRON["VPN_RECORD_PREFIX"]) == 1 { found = 1; exit }
    END { exit(found ? 0 : 1) }
  ' "$file"
}
...
EOF
  ...
  if ! record_exists "\"$VPN_USER\" " /etc/ppp/chap-secrets \
    || ! record_exists "$VPN_USER:" /etc/ipsec.d/passwd; then
cat 1>&2 <<'EOF'
