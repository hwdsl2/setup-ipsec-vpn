...
# Records are generated under .local only. iOS and macOS resolve .local
# service names via unicast DNS when modecfgdomains includes "local".
printf '%s\n' "$RESOLVED" | awk -F';' '
  BEGIN {
...

  {
    ...
    inst_srv[idx] = "srv-host=" fqdn "," host "," p
