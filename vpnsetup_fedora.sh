#!/bin/sh
#
# Script for automatic setup of an IPsec VPN server on Fedora 30.
...
if ! grep -qs -e "Fedora release 30" /etc/redhat-release; then
  exiterr "This script only supports Fedora 30."
fi
...
dnf -y install wget bind-utils openssl \
...
dnf -y install nss-devel nspr-devel pkgconfig pam-devel \
  ...
  xl2tpd \
  libevent-devel fipscheck-devel \
  systemd-devel iptables-services || exiterr2

bigecho "Installing Fail2Ban to protect SSH..."

dnf -y install fail2ban audit-libs-devel libseccomp-devel || exiterr2
